...
#  NLAYERS = 2 
  ...
  CELL_SIZE = params.get(key="cell_size",  default=3) # Internal size of number of hidden layers in each of the LSTM cells
  NLAYERS = params.get(key="rnn_layers",  default=2) # Number of layers RNN stacked cells
  LEARNING_RATE= params.get(key="learning_rate",  default=0.001)
  print ("CELL_SIZE:",  CELL_SIZE)
  print ("NLAYERS:",  NLAYERS)
  print ("LEARNING_RATE", LEARNING_RATE)
  # 0. Reformat input shape to become a sequence
  # Preparing data shape to match rnn fucntion requirements
  # X input shape [BATCH_SIZE, SEQ_LEN,1]
  # Required shape: 'timesteps' tensors list of shape [BATCH_SIZE, number_inputs]

  X =  features[TIMESERIES_COL]

  # labels is [BATCH_SIZE, N_OUTPUTS,1]
  # 1. configure the RNN
  cells = [tf.nn.rnn_cell.BasicLSTMCell(CELL_SIZE, forget_bias=1.0) for i in range(NLAYERS)]
