...
def dropout(input,  pkeep=1):
    output = tf.nn.dropout(input, pkeep)
    return output
    
def mnist_model(activation_function,  hparam,  learning_rate_type, dropout):
...
def make_hparam_string(activation_function,  learning_rate_type,  dropout):
  return "%s_lr_%s_pkeep_%s" % (activation_function,  learning_rate_type,  dropout)
    ...
    for activation_function in activation_functions:    
        for learning_rate_type in learning_rate_types:
            for dropout in dropout_percentages:
                hparam = make_hparam_string(activation_function,  learning_rate_type,  dropout)
                print('Starting run for %s' % hparam)
                # Actually run with the new settings
                mnist_model(activation_function,  hparam,  learning_rate_type,  dropout)
    print('Done training!')
