...
def mnist_model(activation_function, learning_rate,  hparam):
...
def make_hparam_string(activation_function,  learning_rate):
  return "%s_lr_%.0E" % (activation_function,  learning_rate)
...
def main():
    ...
    for activation_function in ["sigmoid","relu"]:    
        for learning_rate in [1E-3, 1E-4]:
            hparam = make_hparam_string(activation_function,  learning_rate)
            print('Starting run for %s' % hparam)
            # Actually run with the new settings
            mnist_model(activation_function,  learning_rate, hparam)
    print('Done training!')
